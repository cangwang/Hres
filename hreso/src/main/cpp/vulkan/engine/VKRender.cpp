...
int VKRender::createRenderPass(VKDeviceManager *deviceInfo, VkFormat vkFormat) {
    //子通道作用与后续的渲染操作，并依赖之前渲染通道输出到帧缓冲区的内容。比如说后处理效果的序列通常每一步都依赖之前的操作。如果将这些渲染操作分组到一个渲染通道中，通过Vulkan将通道中的渲染操作进行重排序，可以节省内存从而获得更好的性能
    // Create render pass
    VkAttachmentDescription attachmentDescriptions {
            .format = vkFormat,
...
int VKRender::createOffscreenCommandPool(VKDeviceManager *deviceInfo,
                                VKBufferManager *bufferInfo, VKOffScreen *vkOffScreenInfo,
                                VulkanFilter *offScreenFilter,
                                VulkanFilter *effectFilter) {
    //创建命令池
    ...
    // 1 command buffer draw in 1 framebuffer
    // In our case we need 2 command as we have 2 framebuffer
    int swapchainLength = vkOffScreenInfo->num;
    cmdBufferLen = swapchainLength;
    cmdBuffer = std::make_unique<VkCommandBuffer[]>(swapchainLength);
    VkCommandBufferAllocateInfo cmdBufferCreateInfo {
            .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
            .pNext = nullptr,
            .commandPool = cmdPool,
    ...
    for (int bufferIndex = 0; bufferIndex < swapchainLength; ++bufferIndex) {
        ...
        // transition the buffer into color attachment
        setImageLayout(cmdBuffer[bufferIndex],
                       vkOffScreenInfo->offscreenPass.color[bufferIndex].image,
                       VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
                       VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                       VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                       VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);

        {
            ...
            FilterFramebuffer effectFrameBuffer{
                    .framebuffer =  vkOffScreenInfo->offscreenPass.frameBuffer[bufferIndex],
                    .width = vkOffScreenInfo->offscreenPass.width,
                    .height = vkOffScreenInfo->offscreenPass.height
            };
            effectFilter->bindFilterFramebuffer(effectFrameBuffer);
